...
def main():
    ...
    if not df_unified.empty:
        top12 = df_unified.nlargest(12, "Investido (R$)")
        cg1, cg2 = st.columns(2)

        with cg1:
            fig1 = go.Figure()
            fig1.add_trace(go.Bar(x=top12["UTM Campaign"], y=top12["Investido (R$)"],
                                  name="Investido", marker_color="#5f12ff"))
            fig1.add_trace(go.Bar(x=top12["UTM Campaign"], y=top12["Net PL (R$)"],
                                  name="Net PL", marker_color="#f7f716"))
            fig1.update_layout(**PLOTLY_LAYOUT, title="Custo vs Net PL por UTM",
                               barmode="group", height=360)
            st.plotly_chart(fig1, use_container_width=True)

        with cg2:
            df_sc = df_unified.dropna(subset=["CPA Real (R$)", "ROAS Net PL"])
            df_sc = df_sc[df_sc["FTDs"] > 0]
            if not df_sc.empty:
                fig2 = px.scatter(
                    df_sc, x="CPA Real (R$)", y="ROAS Net PL",
                    size="FTDs", color="Campanha Meta",
                    hover_name="UTM Campaign",
                    title="CPA vs ROAS por UTM",
                    size_max=40,
                    color_discrete_sequence=["#5f12ff", "#f7f716", "#ff6b6b", "#00ff88",
                                             "#d4b8ff", "#ffa500", "#00bfff", "#ff69b4"],
                )
                fig2.add_hline(y=1.0, line_dash="dash", line_color="#f7f716",
                               annotation_text="Break-even (ROAS 1x)")
    ...
    # ── EVOLUÇÃO DIÁRIA ──────────────────────────────────
    st.subheader("Evolução Diária por UTM")

    df_sm_daily   = fetch_smartico_daily(date_from_str, date_to_str)
    df_meta_daily = fetch_meta_daily(date_from_str, date_to_str)

    if not df_sm_daily.empty:
        top_utms = (df_sm_daily.groupby("utm")["ftds"].sum()
                    .sort_values(ascending=False).head(8).index.tolist())
        utms_sel    = st.multiselect("UTMs", df_sm_daily["utm"].unique().tolist(), default=top_utms)
        metric_opts = {"FTDs": "ftds", "Net PL": "net_pl", "Registros": "regs",
                       "Net Deposits": "net_deposits", "Comissão": "comm_total"}
        metric_label = st.selectbox("Métrica", list(metric_opts.keys()))
        metric_col   = metric_opts[metric_label]
        show_custo   = st.checkbox("Sobrepor custo Meta", value=False)

        df_plot = df_sm_daily[df_sm_daily["utm"].isin(utms_sel)]
        if not df_plot.empty:
            fig3 = px.line(df_plot, x="data", y=metric_col, color="utm",
                           title=f"{metric_label} por UTM — {date_label}", markers=True,
                           color_discrete_sequence=["#f7f716", "#5f12ff", "#00ff88", "#ff6b6b",
                                                    "#d4b8ff", "#ffa500", "#00bfff", "#ff69b4"])
            if show_custo and not df_meta_daily.empty:
                df_meta_agg = (df_meta_daily[df_meta_daily["utm"].isin(utms_sel)]
                               .groupby("data", as_index=False)["custo"].sum())
                fig3.add_bar(x=df_meta_agg["data"], y=df_meta_agg["custo"],
                             name="Custo Meta", marker_color="rgba(95,18,255,0.4)",
                             yaxis="y2", opacity=0.6)
                fig3.update_layout(yaxis2=dict(overlaying="y", side="right",
                                               showgrid=False, title="Custo (R$)",
                                               color="#d4b8ff", titlefont=dict(color="#d4b8ff")))
            fig3.update_layout(**PLOTLY_LAYOUT, height=420)
            st.plotly_chart(fig3, use_container_width=True)

            with st.expander("📋 Tabela pivot diária"):
                pivot = df_plot.pivot_table(index="data", columns="utm",
                                            values=metric_col, aggfunc="sum")
                st.dataframe(pivot.style.format("{:.2f}"), use_container_width=True)
    else:
        st.info("Sem dados diários Smartico para o período.")

    st.divider()

    # ── CONCLUSÕES & PLANO DE AÇÃO ──────────────────────────────────
    st.subheader(f"Conclusões & Plano de Ação — {date_label}")
    conclusions, actions = generate_conclusions(df_unified)

    tab1, tab2 = st.tabs(["📋 Conclusões do Período", "🎯 Plano de Ação"])
    ...
    with tab1:
        if conclusions:
            ...
            st.info("Sem dados suficientes para gerar conclusões.")
        st.text_area("📝 Anotações manuais", placeholder="Adicione observações do período...", height=100)

    with tab2:
        priority_order = ["🔴 Urgente", "🟠 Alta", "🟡 Média", "🟢 Oportunidade"]
        for priority in priority_order:
            prio_actions = [t for p, t in actions if p == priority]
            if prio_actions:
                st.markdown(f"**{priority}**")
                for text in prio_actions:
                    st.markdown(f"- {text}")
        if not actions:
            st.info("Nenhuma ação gerada.")
        action_txt = "\n".join([f"[{p}] {t}" for p, t in actions]) if actions else "Nenhuma ação."
        st.download_button("📥 Exportar Plano de Ação", action_txt.encode(),
                           "plano_acao_multibet.txt", "text/plain")
